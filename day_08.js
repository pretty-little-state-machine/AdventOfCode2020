...
class Cpu {
    // Public API
    constructor(break_on_loop = true) {
        this.acc = 0;
        this.memory = [];
        this.pc = 0;
    }
    ...
    flip_nop_jump(idx) {
        if (this.memory[idx].opcode === "nop") {
            this.memory[idx].opcode = "jmp"
    ...
    run() {
        ...
        this.exec_inst(this.memory[this.pc])
        return { status: ":running", msg: this.pc }
    }
    ...
    dump_breakpoint() {
        let message = "Loop detected! Program counter at: " + this.pc + "\n"
        message += "Accumulator: " + this.acc + "\n"
        message += "Last Instruction: " + JSON.stringify(this.memory[this.pc])
        return message
    }
}
